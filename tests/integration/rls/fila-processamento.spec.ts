 ...
 * `fila_processamento` (a fila assíncrona, chamada `tarefas` até a Fase 3 do
 * Hub, que deu esse nome à checklist do casal) só tem policy de SELECT e
 ...
 */
describe('RLS: fila_processamento', () => {
  ...
  it('membro de outro casamento não consegue excluir a tarefa', async () => {
    await memberB.client.from('fila_processamento').delete().eq('id', taskA.id)
  ...
  it('nem o próprio membro do casamento consegue excluir a tarefa (sem policy de delete pra ninguém além de service_role)', async () => {
    await memberA.client.from('fila_processamento').delete().eq('id', taskA.id)
