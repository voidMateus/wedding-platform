...
/** Menor largura de celular ainda em uso real, mais tamanhos comuns. */
const LARGURAS = [320, 360, 390, 414, 768, 1024, 1280, 1440]

/**
 * Todas as páginas públicas, não só a home.
 *
 * A home era a única coberta, e as demais herdam exatamente os mesmos
 * componentes de cabeçalho, cartão e barra fixa — se o layout quebra numa,
 * quebra em todas. `/rsvp/[code]` fica de fora por exigir um token de convite
 * válido, que este teste não provisiona; o fluxo dela é o mesmo de `/rsvp`.
 */
const CAMINHOS = ['', '/presentes', '/rsvp', '/galeria']
  ...
  try {
    for (const caminho of CAMINHOS) {
      for (const largura of LARGURAS) {
        ...
        const onde = `${caminho || '/'} @ ${largura}px`

        expect(
          medida.scrollWidth,
          `rolagem horizontal em ${onde} (a página pede ${medida.scrollWidth}px)`,
        ).toBeLessThanOrEqual(medida.clientWidth)

        expect(
          medida.encostados,
          `texto a menos de ${MARGEM_MINIMA_PX}px da borda em ${onde}`,
        ).toEqual([])
