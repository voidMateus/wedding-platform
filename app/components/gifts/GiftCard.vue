...
function handlePaymentSubmit(payload: {
  amountCents?: number
  quotaCount?: number
  message: string
}) {
...
<template>
  <UiCard padding="none" radius="xl" elevation="xl" class="flex h-full flex-col overflow-hidden">
    ...
    <div v-else-if="gift.imageUrl" class="relative">
      ...
      >
        {{ formatCentsToBRLOrDash(gift.priceCents) }}
      </span>
    </div>
    <div class="flex flex-1 flex-col gap-3 p-5">
      ...
      <div class="mt-auto flex flex-col gap-3">
        <template v-if="!gift.isGroupGift">
          <p v-if="!gift.imageUrl" class="text-sm font-medium text-text">
            {{ formatCentsToBRLOrDash(gift.priceCents) }}
          </p>
          <div>
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <p class="text-sm text-text-muted">
              <span class="font-medium text-text">{{
                formatCentsToBRLOrDash(gift.collectedAmountCents)
              }}</span>
              de {{ formatCentsToBRLOrDash(gift.targetAmountCents) }} arrecadados
            </p>
          </div>

          <UiButton
            v-if="gift.hasPixOption"
            class="w-full"
            rounded="full"
