...
- **Fase Filtros por Coluna (em andamento)**: recorte de lista deixa de ser fileira de chips e vira filtro/ordenação da própria coluna, com o estado na URL, cabeçalho fixo e múltipla escolha onde faz sentido (dois status, duas faixas). **Entregues**: Convidados (com coluna de Status de RSVP apoiada na view `convidados_com_status`, a única do projeto), Presentes e Plataforma — esta última migrada para `AdminTable`, o que aposentou a `UiTable`. Grupos não entra: é lista com barra de andamento, não tabela. **Falta Convites**, que exige reestruturar a consulta do endpoint, porque `responseStatus`/`memberCount` são calculados depois da paginação e os chips de hoje filtram só a página carregada (achado registrado em [`CHANGELOG.md`](CHANGELOG.md)).
