...
- **Hub, Fase 1 — Financeiro (2026-09-10)**: primeiro módulo do plano de Hub (`docs/plano-produto-hub-casamento.md`). Orçamento em três níveis (categorias → despesas → parcelas), fornecedores com pipeline e cotação, documentos com arquivo no primeiro bucket **privado** do projeto ou link externo, e uma Visão geral organizada pelos quatro estágios do dinheiro. Nenhum estado de pagamento é gravado — tudo deriva de `pago_em`. Refinamento e decisões em [`fase1-financeiro.md`](fase1-financeiro.md).
