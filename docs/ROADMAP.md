...
- **Hub, Fase 5 — Multi-evento e o planejador profissional (2026-09-15)**: a fase que acrescenta um **segundo tipo de gente** ao painel. `membros_casamento.papel` ganha `planejador` ("Assessoria" na tela do casal), numa **escada** — um membro alcança todo papel abaixo do seu, o dono alcança também os outros donos, e ninguém se promove —, com `podeGerenciarPapel()` como autoridade única e par em SQL. O bloco de identidade do cabeçalho vira a troca de evento quando há mais de uma membership, e `/admin` passa a ser a lista de eventos (os próximos primeiro, arquivados por último). `/plataforma` deixa de ser só leitura: **cria casamento**, em transação, com o usuário do dono resolvido fora dela (o convite de e-mail não volta atrás com rollback) e o slug como chave de idempotência — a linha nasce `rascunho`, então criar não publica. `trilha_auditoria` ganha o ator `operador` para que essa criação nunca aconteça sem registro. E o painel interno passa a medir **storage**, em bytes e sob demanda: `contadores_uso` segue deliberadamente vazia. A fase começou por uma **correção estrutural** (F5.0): as chaves de cache do painel eram globais para payload que é por casamento, e o contexto ativo era um `ref` populado só no login — trocar de evento mostraria o dado do anterior, sem erro nenhum. Refinamento e decisões em [`fase5-multievento.md`](fase5-multievento.md).
...
- [x] **Revisão de performance com dados de casamentos grandes** (medida em 2026-09-16, sobre um casamento semeado com **520 convidados**, 190 convites, 347 respostas de RSVP, 40 mesas e 36 presentes). **Volume de dado não é o problema**: a listagem de convidados custa o mesmo na página 1 e na última (374ms × 371ms com 100 por página), a busca por nome custa o mesmo que a listagem, e 200 linhas da view `convidados_com_status` custam o mesmo que 25 (106ms × 97ms). O que domina a latência é o **número de idas ao banco em série** por rota — uma constante da rota, não uma função do tamanho da lista. Números completos no [`CHANGELOG.md`](CHANGELOG.md).
- [x] **LCP do site público** — remedido em 2026-09-16 contra o build de produção, na home com as 11 seções ligadas, perfil Pixel 5 + Slow 4G + CPU 4x (9 amostras): **2,53s de mediana** (2,26–2,79s), contra os 5,8s da Fase Editorial. **A meta de 2,5s está na linha, não vencida com folga.** O achado que explica a diferença é de método: o `node .output/server/index.mjs` local **não comprime nada**, e a medição antiga (como a primeira desta rodada, que deu 4,78s) descrevia uma rede que o convidado nunca usa — a Vercel serve brotli. Medido atrás de um proxy que só acrescenta gzip, a mesma página cai de 983 kB para 415 kB transferidos e o LCP encolhe 2,2s. Fica em aberto **confirmar em campo** contra o deployment real, que a rede corporativa daqui não alcança.
- [x] **Testes de carga** — rodado em 2026-09-16 (`--concorrencia 30 --duracao 20` sobre o casamento de 520 convidados): 25,2 req/s, e a home pública em **243ms de mediana** (p95 644ms). O resultado útil foi o desvio: `/api/public/:slug/gifts` respondia em **3,3s de mediana** sob a mesma carga, 13× a home, por encadear cinco idas ao banco em série. Duas delas eram independentes e passaram a ser paralelas no mesmo dia — **3,28s → 2,44s de mediana e 25,2 → 35,1 req/s de vazão**; as outras estão nomeadas no [`CHANGELOG.md`](CHANGELOG.md), junto do defeito que a execução revelou no próprio script (o termo de busca padrão devolvia 400 em todo pedido daquele cenário). Registrado também que 107 das 559 respostas foram **429**: o rate limiting responde antes da aplicação na busca por nome, e uma medição dessa rota precisa dizer se está medindo o limitador.
...
- [ ] **Página comercial da plataforma** (posicionamento, preços, prova social, cadastro) — aberto em 2026-09-21 pela rodada de usabilidade ([`rodada-usabilidade-2026-09.md`](rodada-usabilidade-2026-09.md), item B4). A raiz do domínio ganha nessa rodada uma **porta**: capa curta com "Entrar", `noindex`, para o casal que digita o domínio em vez de usar o link do e-mail. A página comercial **não é uma versão maior dela** — tem outro público (quem ainda não é cliente), outra métrica e depende de haver o que vender: preço, plano e cadastro self-service. Indexação é decisão dela, não da porta.
- [ ] **Envio automático de convites por WhatsApp Business API** — aberto em 2026-09-21 pela mesma rodada (item F1). A plataforma envia hoje por `wa.me`, que abre a conversa com a mensagem pronta e exige um gesto humano por convite; a rodada acrescenta a **fila guiada** (quem falta, um por vez), que é o máximo que dá para fazer sem API. Envio disparado por gatilho exige conta Business verificada, templates aprovados pela Meta e **custo por conversa iniciada** — por isso depende de billing para caber no preço, e não de esforço de implementação.
- [ ] Painel de administração da plataforma (visão do time interno sobre todos os tenants). Fundação mínima entregue em 2026-08-25 (`docs/PLANO-SAAS.md`, Passo 8) e ampliada na **Fase 5 do Hub** (2026-09-15): `/plataforma` passou a **criar casamento** (com dono vinculado, em transação, auditado como `operador`) e a mostrar **storage medido** por casamento e da plataforma. Faltam suporte e billing — item permanece aberto até os dois chegarem.
- [x] **Papel de "planejador de casamentos" gerenciando múltiplos eventos de clientes distintos** — entregue na Fase 5 do Hub (2026-09-15): papel `planejador` numa escada de três valores, troca de evento no cabeçalho e lista de eventos em `/admin`. O que ficou de fora de propósito: painel agregado com números somados entre eventos (a pergunta boa é "o que precisa de mim hoje", e ela se desenha depois de haver um profissional real usando a lista) e equipe própria por assessoria.
