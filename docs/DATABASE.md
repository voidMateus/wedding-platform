...
| `membros_casamento` | Usuários com acesso administrativo a um casamento (casal, assessoria, colaboradores) — `papel` é uma **escada** de três valores, ver `PRODUCT.md` seção 7.3 |
...
| Tabela | Propósito |
|---|---|
| `categorias_orcamento` | Categoria do orçamento (Buffet, Espaço...) com o valor **planejado**. Taxonomia única do módulo: usada por `despesas` e por `fornecedores`. `cor_indice` é a POSIÇÃO na paleta derivada do tema (nunca a cor), `cor_personalizada` é o override manual |
...
| `trilha_auditoria` | Trilha de auditoria de ações administrativas sensíveis — três tipos de ator: membro, sistema e operador de plataforma |
| `exclusoes_de_casamento` | O que a plataforma excluiu, por quem e quando — **fora do cascade** de `casamentos`, porque a trilha de auditoria morre junto com o casamento e um registro que some com o que descreve não é registro |
...
| `membros_casamento` | `casamento_id`; `usuario_id` (auth) | Acesso administrativo — `papel` (`dono`\|`planejador`\|`colaborador`). Quem alcança quem é `pode_gerenciar_papel(casamento_id, papel_alvo)`, par em SQL de `shared/papeis-de-membro.ts`; as três policies de escrita da própria tabela usam esse predicado. `acesso_suporte_expira_em` (nulo em todo membro real) marca **acesso temporário da equipe interna** e sua validade — checada dentro dos três predicados, então vínculo vencido deixa de valer sem precisar ser apagado |
...
| `trilha_auditoria` | `casamento_id`; `autor_id` → `membros_casamento` (opcional); `autor_operador_id` → `operadores_plataforma` (opcional) | `tipo_autor` (`membro`\|`sistema`\|`operador`), `acao`, `tipo_entidade`/`entidade_id`, `metadados`. O constraint garante que **cada tipo tem exatamente uma forma de autor**: sistema sem nenhuma, membro com `autor_id`, operador com `autor_operador_id` — um operador nunca finge ser membro |
...
- `convidados.convite_id` é o vínculo que habilita RSVP — nullable até o convidado ser vinculado a um convite (criar o convite é ação pedida no cadastro, nunca automática — ver `PRODUCT.md` 3.7), mas obrigatório pra responder RSVP. `convidados.grupo_id` (etiqueta livre) e `convidados.nucleo_id` (Acompanhantes) são **independentes** de `convite_id` e entre si — os três nunca devem ser confundidos (ver seção 2).
- **Independentes não significa livres: um núcleo vive dentro de um convite só, e nunca tem menos de dois membros.** `sincronizar_nucleo_convidado()` recusa (`GUEST_ALREADY_IN_ANOTHER_INVITE`) quando um membro já pertence a outro convite, e `agrupar_acompanhantes()` recusa (`GUESTS_IN_DIFFERENT_INVITES`) a seleção que atravessa convites — nunca move alguém de convite para resolver, o que trocaria o link/QR já compartilhado. `normalizar_nucleo_acompanhantes()` fecha os dois lados: dissolve o núcleo com menos de dois membros (apagando a linha) e adensa `ordem_nucleo` a partir de 0, e é chamada também para o núcleo de **origem** de quem foi movido. `ordem_nucleo` é único por `(nucleo_id, ordem_nucleo)` num índice **parcial** (`where nucleo_id is not null`), então quem não tem núcleo fica todo em 0 sem colidir — e é justamente por isso que `ordem_nucleo` não serve para ordenar um convite (ver `server/utils/membros-do-convite.ts`).
...
- **Financeiro: a cor da categoria é um slot, não uma cor.** `categorias_orcamento.cor_indice` (smallint, default 0) guarda a POSIÇÃO na paleta que `shared/utils/paleta-categorias.ts` deriva da cor tema do casamento; gravar o HEX gerado faria a categoria continuar borgonha depois de o casal trocar o tema para azul. O slot vem do trigger `categorias_orcamento_atribuir_cor` (BEFORE INSERT), que pega o **menor livre entre as ativas** do casamento — nunca hash do id, que colidiria e daria a mesma cor a duas categorias. Arquivar libera o slot para a próxima criada sem repintar as outras, e `categorias_orcamento_cor_ao_restaurar` (BEFORE UPDATE) trata o retorno: slot ainda livre, a categoria volta com a cor de sempre; slot tomado, recebe o menor livre. `cor_personalizada` (nullable, `CHECK` de HEX de 6 dígitos) é o override manual — nulo é o caso normal.
...
- **GRANT de tabela é declarado na migration que cria a tabela, nunca herdado.** A partir de 30/10/2026 o Supabase deixa de conceder acesso da Data API às tabelas novas de `public` — inclusive as criadas por migration, em projeto novo e em `supabase db reset` —, e tabela sem GRANT responde 42501 até para `service_role`. A receita: `service_role` sempre (é quem o servidor usa em todo caminho sem RLS); `authenticated` quando o painel lê ou escreve pela RLS; `anon` **só** quando a tabela tem policy de leitura pública. GRANT é o pré-requisito para a policy ser avaliada, nunca a autorização — quem decide linha a linha continua sendo a RLS.
  ```sql
  grant select, insert, update, delete on table minha_tabela to authenticated, service_role;
  ```
  As tabelas anteriores a 24/09/2026 tinham tudo por herança; `20260924120001_grants_explicitos_de_tabela.sql` declarou o que elas já tinham (DML aos três papéis, no-op em dev/prod). Desde então `supabase/seed.sql` não concede nada — o grant amplo que ele fazia deixava o CI verde para uma tabela que nasceria fechada em prod. A varredura `tests/unit/server/grants-de-tabela.spec.ts` falha quando uma migration cria tabela sem GRANT a `service_role`.
...
- **Funções que leem entre casamentos**: o caminho Plataforma é o único que atravessa tenants, e é sempre `service_role` + checagem explícita em TypeScript (`CLAUDE.md` seção 4.2) — nunca uma policy de RLS cross-tenant. Toda função `security definer` desse caminho nasce com `execute` **revogado de `public`/`anon`/`authenticated`** e concedido só a `service_role`. Hoje são três: `criar_casamento_com_dono()` (cria tenant + dono + trilha numa transação), `uso_de_storage_por_casamento()` (soma `storage.objects`, que o PostgREST nem expõe) e `buckets_contabilizados()`.
- **Métrica de uso é medida, nunca contada por gatilho**: `contadores_uso` continua sem nenhum escritor de propósito. Contador materializado que precisa ser lembrado em cada caminho de escrita erra no primeiro que esquecer — a mesma razão pela qual `mesas.ocupacao` nunca existiu e `convites.status_convite` saiu do schema. Ela volta a fazer sentido quando houver limite de plano a **aplicar na escrita** (Fase 6), que é pergunta diferente de "quanto este casamento ocupa hoje".
