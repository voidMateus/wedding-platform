...
| `membros_casamento` | `casamento_id`; `usuario_id` (auth) | Acesso administrativo — `papel` (`dono`\|`planejador`\|`colaborador`). Quem alcança quem é `pode_gerenciar_papel(casamento_id, papel_alvo)`, par em SQL de `shared/papeis-de-membro.ts`; as três policies de escrita da própria tabela usam esse predicado |
