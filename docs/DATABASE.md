...
|---|---|---|
| `casamentos` | — | Evento — `slug`, `nomes_noivos`, `data_evento`, `prazo_rsvp`, `modo_lista_convidados`, `idade_maxima_crianca`, `config_tema`, `config_conteudo`, `status_ciclo_vida`, `arquivado_em` |
| `membros_casamento` | `casamento_id`; `usuario_id` (auth) | Acesso administrativo — `papel` |
| `etapas_evento` | `casamento_id`; `mesmo_local_que` (auto-referência, opcional) | Cerimônia/recepção/festa — local, horário, `ordem_exibicao` |
...
- `is_child` **não é uma coluna** — é sempre calculado por `convidado_e_crianca(data_nascimento, casamento_id)` a partir de `convidados.data_nascimento` + `casamentos.idade_maxima_crianca` (default 11 anos). Sem `data_nascimento`, o convidado conta como adulto.
