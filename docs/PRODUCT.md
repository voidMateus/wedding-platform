...
**Três canais: WhatsApp, e-mail e "outro".** O WhatsApp é o canal real do casamento brasileiro: o botão abre a conversa com a mensagem pronta e o link do convite, e **abrir o WhatsApp É a declaração de envio** — o registro é gravado no mesmo gesto. Voltar da aba para confirmar cada um dobraria os gestos numa tarefa que se faz oitenta vezes, e o sistema nunca comprovou entrega, nem antes. O **e-mail** sai pelo servidor e é o único canal em que o envio é confirmado de verdade: só há registro se o provedor aceitou a mensagem, e o que acontece depois (entregue, devolvido, marcado como spam) chega por webhook e vira log próprio. O canal `outro` é o convite entregue em mãos, mandado pela cerimonialista ou pelo correio: **registrar um envio feito fora da plataforma é caminho de primeira classe**, pelo mesmo motivo que a resposta registrada pelo casal é (seção 5.2) — o funil nunca exigiu jornada digital.

**O canal é uma sessão de trabalho, não uma escolha por linha.** O par "o que estou mandando" + "por onde" governa a tela inteira: é ele que decide qual contato a coluna mostra, quantos estão sem contato e o que o botão de cada linha faz. Sem provedor de e-mail configurado no ambiente, o seletor de canal **some** — o recurso não existe naquela instalação, e um seletor de uma opção só é ruído.

**Só o que não chegou aparece.** Um e-mail devolvido ou marcado como spam é sinalizado na linha e contado no topo ("3 não chegaram"), porque é a única informação que o sistema tem e o casal não. "Entregue" não vira selo: a tela ficaria coberta de confirmações de que o normal aconteceu.

**Registro de canal `outro` tem volta; envio feito pelo sistema não.** Declarar por engano precisa ter saída; apagar um WhatsApp ou um e-mail que saiu seria reescrever a história. A assimetria é a mesma de "*Aberto* é o único estágio comprovado pelo sistema", e mora na policy de RLS, não só na ausência de um botão.

**Uma mensagem por tipo, compartilhada entre canais**, editável pelo casal com um catálogo fechado de variáveis (`{{nome}}`, `{{casal}}`, `{{data}}`, `{{local}}`, `{{prazo}}`, `{{link}}`). Texto em branco usa o padrão da plataforma, que é conteúdo de verdade e não placeholder — quem nunca abrir o editor manda uma mensagem que se sustenta. Variável fora do catálogo fica literal no texto (visível e corrigível); variável conhecida sem valor vira vazio, porque um casamento sem local cadastrado não pode travar o envio do convite. **O e-mail usa esse mesmo texto**, dentro de um layout com a cor do casamento — e não acrescenta botão de ação: o link fica onde o casal o escreveu, na frase dele, e um botão embaixo repetiria o mesmo endereço duas vezes na mesma mensagem.
...
**Convite sem contato não é erro, é trabalho a fazer.** A tela conta quantos são — pelo canal de agora, porque o mesmo convite pode ter e-mail e não ter telefone — e leva para completar os cadastros; o botão da linha vira "Registrar" em vez de sumir.

### 5.4.1 Avisos automáticos — a única coisa que a plataforma faz sozinha

**O princípio: nasce desligado.** Em todo o resto do produto o padrão é conteúdo pronto que já funciona sem ninguém configurar. Aqui é o contrário, e de propósito: o padrão escreveria para a caixa de entrada de oitenta pessoas com o nome do casal no remetente. Ligar é decisão dele, em Configurações › Avisos, e a tela diz a data do próximo envio **antes** de qualquer coisa sair.

**Dois avisos, dois públicos.** O *lembrete de RSVP* vai ao **convidado** — para quem já recebeu o convite e ainda não respondeu por inteiro. O *aviso de vencimento* vai ao **casal**, com as parcelas que vencem em breve, num e-mail só por dia (três contas na mesma semana são uma notícia só) e assinado pela plataforma, não pelo casal: uma mensagem com o nome deles chegando na caixa deles pareceria phishing.

**Marcas, não frequência.** O casal escolhe quantos dias antes (até três marcas, ex.: 14 e 3), e o aviso sai no dia exato de cada uma. **Nada dispara depois que a data passou**: prazo vencido não vira cobrança diária, e parcela vencida já aparece em vermelho na tela de Pagamentos todo dia — sem precisar de e-mail todo dia.

**Ninguém recebe dois no mesmo dia, e quem respondeu sai da lista sozinho.** As duas garantias são derivadas dos fatos (o log de envios do dia; as respostas registradas), nunca de uma coluna de controle a manter sincronizada. E o lembrete de RSVP só sai de casamento **publicado** — ele leva um link para o site, e mandar o convidado para um site fora do ar é pior que não lembrar.
...
**Acesso de suporte da plataforma.** A equipe interna entra no painel de um casamento pelo `/plataforma`, e entra como **membro de verdade, temporário** — `membros_casamento` com `acesso_suporte_expira_em` preenchido, papel `dono`, validade de 4 horas. Não existe bypass: 123 das 128 rotas do painel autorizam por RLS, então um contexto especial não abriria nada, e uma policy cross-tenant é proibida (`CLAUDE.md` seção 4.2). A validade **vale na leitura**, dentro de `is_membro_casamento` — vínculo vencido para de abrir o painel no instante em que vence, mesmo antes de a linha ser apagada. O vínculo **não aparece** nesta tela de acessos e **não conta** como dono; conceder e encerrar ficam registrados na trilha do casamento, que o casal também lê. Detalhes: [`fase5-multievento.md`](fase5-multievento.md) seção 6.7.
