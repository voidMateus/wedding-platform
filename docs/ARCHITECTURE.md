...
│   │   └── admin/
│   │       ├── guests/                # GuestPartyModal, GuestPersonFields
...
Um terceiro ambiente (`staging`, entre `dev` e `prod`) é uma extensão natural — mesmo padrão de projeto Supabase dedicado, promovido automaticamente a cada merge em `main`, servindo de gate antes da promoção para `prod` — mas não foi criado nesta primeira fase de produção; adicionar depois é só repetir o mesmo processo descrito abaixo com o projeto novo, sem mudança estrutural.
...
3. Promoção para `prod` é **automática no merge em `main`**, executada pelo job `migrate-prod` do CI (`.github/workflows/ci.yml`) — `supabase db push --include-all` contra o secret `SUPABASE_PROD_DB_URL`, nunca da máquina de um desenvolvedor. Foi um passo manual e deliberado até 2026-09-09; duas razões inverteram a decisão. (a) Manual dependia de alguém lembrar do passo *depois* do merge, e o deploy de produção da Vercel não espera por ninguém — código no ar antes do schema que ele exige é falha real, já vista: a `20260824090001_operadores_plataforma` atrasada derrubou o login inteiro em produção (`resolvePlatformOperator()` roda para todo usuário autenticado), não só `/plataforma`. (b) O gate de verdade de uma migration nunca foi a promoção: é a execução de `pull_request` deste mesmo CI, que aplica todas elas contra um Postgres real e roda a suíte de integração antes do merge. "Manual" não significava mais revisado, só mais tarde — e `main` é branch protegida, então nada chega ao job sem esse gate ter passado. O job deliberadamente **não** declara `needs: ci`, para não perder a corrida contra o build da Vercel; a justificativa completa está em comentário no próprio workflow. (Se/quando `staging` existir como terceiro ambiente, entra como promoção intermediária no mesmo job, antes de `prod` — ver 4.1.)
