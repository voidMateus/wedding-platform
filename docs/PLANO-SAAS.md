...
- **Fase atual**: Passo 1 **completo, mergeado em `main` e promovido para produção**. Passo 2 (suíte de testes de isolamento) **completo e validado** — PR #84, aguardando autorização explícita de merge. Passo 3 (Conta → Casamento) **parcialmente implementado** — PR #85 (contexto multi-membership + endpoints de gestão de membros); restructuring de rotas/UI de seleção/E2E ainda pendentes. Passo 4 **parcialmente implementado e validado** — PR #86, itens 1-3 já vinham do Passo 1, item 4 (RLS das tabelas de SaaS-readiness) implementado e validado contra `dev` (usuário aplicou a migration manualmente); itens 5-6 deliberadamente não iniciados (sem call site real ainda). Hotfix isolado dos 2 bugs de produção do Passo 2 em PR #83.
- Executados em sequência sob autorização explícita do usuário ("Realizar Passos 2, 3, 4 / Sem interrupção") em 2026-08-21 — ver Log de progresso para detalhes técnicos de cada um.
- **PRs abertos, nenhum mergeado** (aguardando checkpoint final): #83 (hotfix, base `main`) → #84 (Passo 2, base `main`) → #85 (Passo 3 parcial, base `main`) → #86 (Passo 4 parcial, base `feature/passo2-testes-integracao`, empilhado sobre #84 por depender dos arquivos de spec que ele introduz). #83 e #84 compartilham o commit do hotfix (extraído pra #83 poder ser mergeado independentemente) — o diff de #84 correspondente fica automaticamente vazio assim que #83 mergear.
...
- **Merge e produção do Passo 1 (2026-08-21)**: os 4 PRs da pilha (#79→#80→#81→#82) foram mergeados em `main` nessa ordem, com autorização explícita do usuário. As 5 migrations de rename foram aplicadas ao projeto `prod` (`elatoqglxrpqriqphkjy`) logo em seguida. Deploy do Vercel do commit final (`9ef60ef`) concluído com sucesso; usuário confirmou produção funcionando normalmente. Houve uma janela curta (~1-2 min) entre o deploy do código novo e a aplicação da migration em prod, devido a retentativas do classificador de permissão do Claude Code nos comandos `gh pr merge`/`supabase db push` — sem impacto observado, dado que o ambiente ainda está pré-lançamento (sem dado real de casamento).
- **Pendência ativa de risco**: a migration `20260821110001` (correção do bug real que quebrava o wizard de criação de convidado) está aplicada em `dev` mas **ainda não em `prod`** — o bug está ao vivo em produção até essa migration ser promovida, o que depende de autorização de merge/deploy deste lote de trabalho.
