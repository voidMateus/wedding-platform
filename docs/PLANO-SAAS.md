...
- **Fase atual**: Passo 1 **completo, mergeado em `main` e promovido para produção**. Passo 2 (suíte de testes de isolamento) **completo, validado e mergeado**. Passo 3 (Conta → Casamento) **parcialmente implementado e mergeado** (contexto multi-membership + endpoints de gestão de membros); restructuring de rotas/UI de seleção/E2E ainda pendentes. Passo 4 **parcialmente implementado, validado e mergeado** — itens 1-3 já vinham do Passo 1, item 4 (RLS das tabelas de SaaS-readiness) validado contra `dev` e `prod`; itens 5-6 deliberadamente não iniciados (sem call site real ainda).
