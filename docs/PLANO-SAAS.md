...
- **Fase atual**: Passo 1 **completo, mergeado em `main` e promovido para produção**. Passo 2 (suíte de testes de isolamento) **completo, validado, ainda não mergeado** — branch `feature/passo2-testes-integracao`, aguardando autorização explícita de merge (contrato de execução, seção acima). Passo 3 (Conta → Casamento) **parcialmente implementado** (contexto multi-membership + endpoints de gestão de membros prontos; restructuring de rotas/UI de seleção/E2E pendentes) — ainda não commitado, será movido para branch própria. Passo 4 ainda não iniciado.
