...
- **Polimento "Admin Premium" (reversão parcial da decisão acima)**: o admin deixou de ser puramente estático — cards, botões, campos de formulário e linhas de tabela ganharam transições suaves (`transition-brand`, ver 22.1) e estados de hover no degrau médio da escala de sombra/raio (`shadow-md`/`radius-md`, antes reservados ao site público). A régua permanece a mesma de sempre: **sutil e profissional, nunca decorativo** — o tratamento pill/glow/uppercase de `rounded="full"` (CTAs do site público) continua fora do admin, que segue em `rounded="md"` sem scale/uppercase (testado em `Button.spec.ts`). Cabeçalho de página padronizado via `AdminSection.vue` (`components/admin/`, slots `title`/`description`/`actions`); métricas do dashboard via `AdminStatCard.vue` (ícone + label + valor); atalhos de ação via `AdminQuickAction.vue`. Configurações reorganizada em abas (`UiTabs`, ver 22.2) — Geral/Aparência — com as subseções de Aparência agrupadas em `UiAccordion` por tema (Branding/Tema/Experiência) em vez de um formulário único e longo.
...
- **Raio de borda e sombra**: escala limitada (`--radius-sm/md/lg/xl`, `--shadow-sm/md/lg/xl`) fixa na plataforma — não varia por tema, aplicada consistentemente via os componentes de `components/ui/`; nenhum valor arbitrário de `border-radius`/`box-shadow` direto em componentes de domínio. O tier `xl` ("Fase Linguagem Visual") é reservado para cartões de destaque do site público (RSVP, cronograma, presentes) — nunca usado no admin. Os tiers `md`/`lg`, por outro lado, passaram a ser usados também no admin em estados de hover/interativo (`Card` `variant="interactive"`, `Button` `hover:shadow-md`) desde a "Fase Admin Premium" — só o tier `xl` permanece exclusivo do site público (§21).
...
|---|---|
| `Button` | Variantes: `primary`, `secondary`, `outline` (borda sutil `border-primary/25` + fundo translúcido `bg-surface-elevated/70` com `backdrop-blur-sm` — CTAs secundários sobre fundo claro ou foto), `ghost`, `destructive`; tamanhos `sm/md/lg`; prop `rounded` (`'md'` default — admin/formulários; `'full'` = pill — convenção do site público). CTAs em pill (`rounded="full"`) ganham `hover:scale-[1.03] active:scale-95`; quando também `variant="primary"`, um glow colorido (`box-shadow` via `color-mix(in srgb, var(--color-primary) ...)`) — referência de estilo mimodocasal.com.br, "Fase Linguagem Visual". Botões `rounded="md"` (padrão do admin) ficam de fora dessa decoração (sem scale/uppercase) — escaneabilidade prevalece (§21) — mas ganharam polimento próprio na "Fase Admin Premium": `hover:shadow-md` (variante `primary`) e `active:scale-[0.98]`, via `.transition-brand` (§22.1). Prop `to` (renderiza como `NuxtLink` em vez de `<button>`, mesmas classes de variante) + `target` (só com `to`, ex.: `"_blank"`, aplica `rel="noopener noreferrer"` automaticamente) |
...
| `Card` | Contêiner padrão para itens de lista (convidado, presente); props `radius` (`'lg'` default, `'xl'` = tratamento premium — raio/sombra maiores, cartões de vitrine do site público como `GiftCard`), `elevation` (`'sm'` default, `'xl'` acompanha o `radius="xl"`) e `variant` (`'default'` | `'interactive'` — hover `shadow-md`/`border-primary` no degrau médio, "Fase Admin Premium" | `'highlight'` — leve ênfase `bg-primary/[0.03]` para cards de destaque, ex. prazo de RSVP no dashboard). O admin nunca usa `radius="xl"`/`elevation="xl"` — mantém o padrão funcional/denso (§21), mas usa `variant="interactive"`/`"highlight"` livremente |
...
- **Tier de cartão "premium"** (`--radius-xl`/`--shadow-xl`, `UiCard` props `radius`/`elevation`): aplicado a `EventSpotlight` (card de "O Grande Dia"), `RsvpTeaserSection`, `SaveTheDateCard` e `GiftCard` — raio e sombra maiores/mais suaves que o resto da plataforma, reservado ao site público (nunca ao admin).
