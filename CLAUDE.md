...
- **Raio de borda e sombra**: escala limitada (`--radius-sm/md/lg/xl`, `--shadow-sm/md/lg/xl`) fixa na plataforma — não varia por tema, aplicada consistentemente via os componentes de `components/ui/`; nenhum valor arbitrário de `border-radius`/`box-shadow` direto em componentes de domínio. O tier `xl` ("Fase Linguagem Visual") é reservado para cartões de destaque do site público (RSVP, cronograma, presentes) — nunca usado no admin, que continua em `lg`/`sm` (§21).

### 22.2 Componentes base (`components/ui/`)

| Componente | Responsabilidade |
|---|---|
| `Button` | Variantes: `primary`, `secondary`, `outline` (borda sutil `border-primary/25` + fundo translúcido `bg-surface-elevated/70` com `backdrop-blur-sm` — CTAs secundários sobre fundo claro ou foto), `ghost`, `destructive`; tamanhos `sm/md/lg`; prop `rounded` (`'md'` default — admin/formulários, sem decoração extra; `'full'` = pill — convenção do site público). CTAs em pill (`rounded="full"`) ganham `hover:scale-[1.03] active:scale-95`; quando também `variant="primary"`, um glow colorido (`box-shadow` via `color-mix(in srgb, var(--color-primary) ...)`) — referência de estilo mimodocasal.com.br, "Fase Linguagem Visual". Botões `rounded="md"` (padrão do admin) ficam de fora dessa decoração — escaneabilidade prevalece (§21). Prop `to` (renderiza como `NuxtLink` em vez de `<button>`, mesmas classes de variante) + `target` (só com `to`, ex.: `"_blank"`, aplica `rel="noopener noreferrer"` automaticamente) |
...
| `Card` | Contêiner padrão para itens de lista (convidado, presente); props `radius` (`'lg'` default, `'xl'` = tratamento premium — raio/sombra maiores, cartões de vitrine do site público como `GiftCard`) e `elevation` (`'sm'` default, `'xl'` acompanha o `radius="xl"`). O admin nunca usa `'xl'` — mantém o padrão funcional/denso (§21) |
...
- **Ilustrações botânicas opcionais** (`showHeroBotanicals`, novo campo do `theme_config`, default `true`, toggle em Aparência — pedido: "opção de ter os lírios ou não"): `BotanicalSpray.vue` (spray aéreo estilo mosquitinho, lateral esquerda) e `BotanicalBranch.vue` (ramo folhado, canto superior direito do Hero + cantos inferiores das boas-vindas), traço fino em `currentColor`, quase imperceptíveis. Substituem o `HeroCornerBranch.vue` da Rodada 5 (removido). `HeroFlourish.vue` redesenhado como raminho de louro horizontal.
