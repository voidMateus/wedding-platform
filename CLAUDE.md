...
- **No Financeiro, o dinheiro tem três níveis e nenhum estado gravado.** `categorias_orcamento` (o planejado) → `despesas` (o contratado) → `parcelas_despesa` (o caixa e o futuro). `parcelas_despesa.pago_em` é a **única** fonte do estado de pagamento — "paga/a vencer/vencida" é sempre derivado dele e de `vence_em` contra hoje, nunca uma coluna de status. **"A pagar" é contratual** (`valor da despesa − pago`, piso em zero), nunca a soma das parcelas em aberto: parcelamento incompleto é o caso normal ("entrada paga, resto a combinar"), e `a pagar = agendado + não parcelado`. A soma das parcelas **pode** divergir do valor da despesa nos dois sentidos — a divergência é exibida, nunca bloqueada. Todo **piso em zero é aplicado por linha, antes de somar** (despesa a despesa, categoria a categoria): somar primeiro faz uma despesa paga a mais compensar outra em aberto e apaga as duas anomalias. "A contratar" (`previsto − contratado`) e "acima do planejado" (`contratado − previsto`) são dois números independentes, nunca o mesmo com sinal trocado, e **categoria sem previsto nunca acusa estouro**. Indicador sem base (planejado zero, teto ausente) é omitido, jamais exibido como 0% — o resumo degrada, nunca mente. Cálculo só em `shared/utils/orcamento.ts`, com "hoje" recebido como parâmetro (resolvido no fuso do evento, não no do servidor). Fornecedor **nunca** tem estágio "pago" (a situação financeira é derivada das parcelas) e sua cotação (`valor_proposto_centavos`) não entra em total nenhum. `casamentos.orcamento_total_centavos` (teto global) e a soma dos previstos por categoria são números distintos — um nunca é derivado do outro. Nenhum dado do Financeiro tem rota pública. Detalhes: `docs/fase1-financeiro.md`.
