...
Primeiro consumidor real das peças de fundação, reconstrução completa de `Hero.vue` (não um ajuste incremental): as duas variantes (com/sem foto de capa) mantêm a mesma composição geral — selo do casal no topo, nomes grandes centralizados, uma faixa "Quando & onde" e os atalhos do Hero abaixo — mas cada peça agora vem do sistema em vez de markup solto.

- **Faixa "Quando & onde"**: substitui a linha simples de texto ("data • local") por um bloco com borda (`rounded-lg`, cartão de convite impresso — mesma linguagem de `Frame.vue`/`OrnamentMark.vue`) dividido em duas colunas (data/local à esquerda, contagem regressiva à direita, separadas por um filete) — um dispositivo estrutural que encodifica a informação real (o quê é prático vs. o quê é emocional), não decoração solta. Sempre renderiza (a data sempre existe); a coluna de contagem só aparece quando `showCountdown` está ligado.
- **Foto de capa**: usa `UiTreatedImage` (`full-bleed`, `overlay="full"`) em vez do `NuxtImg` + gradiente manual de antes — o overlay `full` foi ajustado de `bg-black/40` para `bg-black/50` (`TreatedImage.vue`) porque o conteúdo agora se espalha por toda a foto (não só ancorado embaixo como antes), então precisa de contraste uniforme, não só um gradiente mais escuro na base.
- **Variante sem foto**: ganhou `UiTexture` (grão sutil em `--color-secondary`) e um wash de gradiente muito leve (`from-secondary/[0.06]`) como fundo — profundidade tátil sem introduzir uma cor nova, mantendo `--color-surface` como base (nunca substituído, só uma camada por cima).
- **Tipografia**: nomes do casal aumentam de escala (`text-5xl sm:text-6xl` com foto → `text-6xl sm:text-7xl` sem foto, ambos de `text-4xl sm:text-5xl`/`text-5xl sm:text-6xl` antes) com `leading-none` em vez de `leading-tight`, para mais presença cinematográfica.
