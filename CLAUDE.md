...
- `event_segments.same_venue_as` (auto-referência, `on delete set null`) resolve o caso de cerimônia e recepção no mesmo local — quando definido, os campos `venue_name`/`venue_address`/`venue_latitude`/`venue_longitude` deste próprio registro ficam sempre nulos (fonte de verdade única, evita duas cópias divergentes do mesmo endereço). Validado na aplicação (`server/utils/validate-same-venue.ts`): não pode ser o próprio id, e não pode apontar para um segmento que já tem `same_venue_as` definido (só um nível de indireção, nunca uma corrente). Excluir um segmento referenciado por outro é bloqueado até o dependente ser desvinculado — checado explicitamente no handler antes do `DELETE`, não só confiando na mensagem de erro da FK. No site público (Fase Premium Experience/PR1), `app/utils/group-event-segments-by-venue.ts` funde o card do dependente no card do segmento referenciado (`PublicEventSpotlight` passa a receber `segments: EventSegment[]`, não mais um único segmento) — Cerimônia e Recepção no mesmo endereço viram uma única seção com dois "momentos" (badge/horário) e um só mapa/endereço/botão, em vez de duas seções duplicadas.
...
- **`CoupleMonogram.vue`** (`app/components/public/`) — o selo do casal: iniciais derivadas de `couple_names` via `shared/utils/couple-initials.ts#getCoupleInitials` (mesma convenção de split por `"&"` já usada em `Hero.vue#coupleNameParts`, com fallback para nomes fora do padrão `"Nome & Nome"`) dentro de um anel duplo, SVG puro. Puramente decorativo (`aria-hidden`) — o nome completo do casal sempre aparece como texto em algum outro lugar da página, o monograma nunca é a única forma de identificar o casal. Ganhou a prop `inverted` no PR4 (Hero), que troca o traço/preenchimento para branco quando usado sobre a foto de capa — as cores primária/secundária do tema são validadas por contraste contra `--color-surface` (seção 22.4), não contra uma foto arbitrária, então não podem ser assumidas legíveis ali.

Nenhuma seção visível mudou de aparência neste PR — as peças de fundação só ganharam consumidor real a partir do Hero (PR4).

#### Hero cinematográfico (PR4)

Primeiro consumidor real das peças de fundação, reconstrução completa de `Hero.vue` (não um ajuste incremental). Uma primeira versão (full-bleed com overlay escuro + faixa "Quando & onde" combinando data/local e contagem) foi **substituída** ainda dentro do mesmo PR depois que o usuário trouxe uma imagem de referência concreta (sem site real associado — a própria imagem é o alvo) mostrando uma composição diferente; o design final segue essa referência:

- **Layout em split, não overlay**: com foto de capa, o Hero vira duas colunas no desktop (`lg:grid-cols-2`) — conteúdo à esquerda sobre `bg-surface`, foto sangrando à direita (`UiTreatedImage`, `full-bleed`, `overlay="none"`, com uma faixa de transição suave `bg-gradient-to-r from-surface to-transparent` na borda entre as duas). No mobile, empilha (foto no topo, ~42vh, conteúdo abaixo) — o texto **nunca fica sobreposto à foto**, elimina de vez o problema de contraste texto-sobre-foto-arbitrária que a v1 deste PR tentava resolver com overlay escuro. Por isso `CountdownTimer`/`CoupleMonogram` nunca precisam de `inverted` neste layout (a prop continua existindo e testada, só não é consumida aqui).
- **Selo do casal** (`PublicCoupleMonogram`) agora também aparece no `NavBar.vue`, ao lado do nome do casal (tamanho `sm`) — só quando `coupleNames` está definido (o fallback "MeuSiteCasamento" não gera monograma).
- **Nomes com "&" diferenciado**: o "&" entre os nomes usa `text-secondary` e uma escala menor (`text-3xl sm:text-4xl` vs. `text-5xl sm:text-6xl` dos nomes) — detalhe tipográfico da referência, não estava na v1.
- **Citação opcional**: `theme_config.heroQuote`/`heroQuoteAttribution` (`shared/schemas/theme.ts`, ambos opcionais, string vazia = não definido — mesmo tratamento de `titleColor`/`bodyColor`) — quando preenchida, aparece como `<blockquote>` itálico logo abaixo dos nomes, precedida por `UiSectionDivider`; a atribuição (ex.: uma referência bíblica) só aparece se a citação também estiver definida. Campo novo em `/admin/configuracoes` (Aparência), `UiTextarea` + `UiInput`. Sem preenchimento (a maioria dos casamentos), o bloco inteiro não renderiza — nunca um espaço vazio.
- **Cartão de contagem regressiva com rótulo "Faltam"**: a contagem (`variant` configurável do PR2) agora vive dentro de um cartão próprio (`rounded-lg border bg-surface-elevated shadow-md`) com um rótulo centralizado acima — não mais solta na página nem combinada com data/local.
- **Data/local em linha própria**: ícone de calendário + data (uppercase, tracked) + nome do local (quando o cronograma já tem um segmento com `venue_name`) — voltou a ser uma linha simples e discreta, separada da contagem (a v1 combinava as duas num único bloco; a referência as mostra separadas).
- **`UiBotanicalSprig.vue`** (`components/ui/`, novo): galho decorativo em traço fino (`currentColor`, prop `flip` para espelhar), versão **simplificada e abstrata** da ilustração floral da referência (decisão explícita do usuário — não uma flor específica reconhecível). Só visível em `lg:` (evita ruído/clutter no mobile), posicionado no canto superior-esquerdo do bloco de conteúdo.
- **Variante sem foto**: mantém `UiTexture` (grão sutil em `--color-secondary`) como fundo, sem o wash de gradiente adicional da v1 (removido por simplicidade ao reconciliar com a referência).
- **Motion**: inalterado deliberadamente (mesma entrada simples de fade/slide de antes) — orquestração de entrada é escopo do PR de Motion (PR8), não deste PR.

**Não validado neste PR** (limitação do ambiente, não decisão de design): a variante com foto de capa não pôde ser conferida ao vivo contra um casamento real (o único casamento real disponível não tem foto de capa cadastrada), e não houve QA de mobile num navegador de verdade — ambos cobertos só por teste unitário. Recomendado revisar visualmente antes do PR5.

**Achado crítico (pós-merge, reportado pelo usuário): `heroQuote` "não estava salvando"** — `server/api/wedding/theme.patch.ts` montava o `theme_config` a salvar escolhendo campos um a um (`presetId`, `primaryColor`, ..., `heroFeaturedButton`), em vez de reaproveitar o objeto já validado por `themeConfigSchema`. Todo campo novo adicionado ao schema depois que essa lista foi escrita ficava órfão — não só `heroQuote`/`heroQuoteAttribution` (PR4), mas também `countdownStyle` (PR2), que tinha exatamente o mesmo problema sem nunca ter sido percebido (não há teste cobrindo esse handler — nenhum `server/api/**/*.patch.ts`/`*.post.ts` deste projeto tem hoje). Corrigido substituindo a lista manual por `{ ...current.theme_config, ...input, presetId: input.presetId || undefined }` — `input` já é o objeto validado inteiro, então um campo novo no schema passa a ser persistido automaticamente, sem exigir lembrar de atualizar este handler a cada vez. Validado com um round-trip real (login via `@supabase/ssr` + PATCH autenticado contra o dev server, não só leitura do código) antes de fechar o achado como corrigido — a mesma técnica fica registrada aqui como referência: para depurar/validar qualquer endpoint autenticado deste projeto sem um navegador disponível, autentique com `supabase-js` (`signInWithPassword`), replique a sessão através de `createServerClient` do `@supabase/ssr` (mesma lib que `@nuxtjs/supabase` usa por baixo) para gerar os cookies exatos que o servidor espera, e use esses cookies num `fetch`/`curl` direto contra as rotas de `server/api/**`.

**Segunda rodada de crítica visual (pós-merge, screenshot real comparado à referência, usuário pediu "elegante, nada brega")**:
- **Contagem regressiva "caixa dentro de caixa"**: a v1 do cartão "Faltam" sempre envolvia a contagem numa caixa elevada, mas `variant="cards"` do `CountdownTimer` já desenha sua própria caixa por unidade — duas camadas de caixa liam como confuso/genérico, não elegante. Corrigido: o cartão elevado (`rounded-lg bg-surface-elevated shadow-lg`, sem borda — só sombra, mais suave) só envolve a contagem quando `countdownStyle === 'inline'`; com `'cards'`, é só o rótulo "Faltam" acima das caixas nativas, sem wrapper extra. `variant="inline"` do `CountdownTimer` também perdeu o separador "·" entre unidades (lia como ruído) e passou a usar `text-primary`/`text-secondary` (antes `text-heading`/`text-text-muted`) para casar com os números vermelhos/rótulos dourados da referência. `theme_config.countdownStyle` deste casamento foi ajustado para `'inline'` para casar com a referência.
- **`UiBotanicalSprig` ilegível**: no tamanho/posição original (`h-32`, offset `-left-4 -top-2`, opacidade 40%), o corte pela borda do Hero deixava só um traço fino solto visível — lia como elemento quebrado, não como ilustração intencional (risco real de "brega" apontado pelo usuário). Aumentado para `h-56`, offset `-left-10 -top-10`, opacidade 50%, traço mais grosso (1.5 em vez de 1.25) e duas folhas extras — mais massa visível dentro da área não cortada. O container do conteúdo do Hero ganhou `overflow-hidden` (clip previsível na borda da seção, evita influenciar o `scrollWidth` da página).
- **Proporção do split**: `lg:grid-cols-2` (50/50) apertava demais a coluna de conteúdo para a fileira de 4 atalhos; ajustado para `lg:grid-cols-[55%_45%]`.
- **Ritmo vertical**: gaps uniformes (`gap-4`) em todos os blocos liam como denso; aumentado para `gap-6` no bloco de conteúdo principal, mais respiro entre selo/nomes/citação/contagem/atalhos.
...
- **PR3 — Fundação** ([x] concluído, PR #59): peças de fundação da Assinatura Visual (sistema de espaçamento, sistema de ornamentos, `UiTreatedImage.vue`, `CoupleMonogram.vue`) — detalhado na seção 22.5. Nenhuma seção visível mudou de aparência ainda.
- **PR4 — Hero cinematográfico** ([x] concluído): reconstrução completa do `Hero.vue`, primeiro consumidor real das peças do PR3 — detalhado na seção 22.5.
