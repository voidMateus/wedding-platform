...
- View nova **precisa** de `security_invoker = true` — sem isso roda com privilégio do dono (ignora RLS). A regra vem de um achado de segurança real (`docs/CHANGELOG.md`). Existe uma única view hoje, `convidados_com_status` (status de RSVP resolvido para a listagem do admin, onde "pendente" inclui quem nunca respondeu): view é o último recurso, para o que o PostgREST não expressa a partir da tabela — nunca conveniência de consulta.
