...
// Presente de cota (isGroupGift) não tem priceCents — o "preço"
...
// é uma entidade nova, é um presente de cota com displayStyle='emocional'.
export function segmentGifts<T extends SegmentableGift>(gifts: T[]): GiftSegments<T> {
  ...
  for (const gift of gifts) {
    if (!gift.isGroupGift) {
      physical.push(gift)
    } else if (gift.displayStyle === 'emocional') {
      emotional.push(gift)
    } else {
