...
-- 12.1) — mesma lógica para criar e editar: recebe o convidado principal,
-- a lista atual de acompanhantes (ordem do array = party_order) e os
-- acompanhantes removidos nesta sessão, e sincroniza tudo (guests +
-- guest_parties + opcionalmente invites) numa única transação. Nunca exclui
-- fisicamente um acompanhante removido — só desvincula (party_id = null).
--
-- Payloads de convidado (p_primary e cada item de p_companions) usam as
-- mesmas chaves em camelCase do corpo da requisição: id?, fullName,
-- nickname?, sex?, birthDate?, photoPath?, weddingRole?,
-- dietaryRestrictions?, notes?, groupId?.

create function sync_guest_party(
  p_wedding_id uuid,
  p_primary jsonb,
...
begin
  ...
  if p_invite is not null then
    ...
    -- bloqueia se algum membro do grupo já pertence a OUTRO convite
    if exists (
...
comment on function sync_guest_party(uuid, jsonb, jsonb, uuid[], jsonb) is
  'Sincroniza convidado principal + Acompanhantes (+ opcionalmente cria/atualiza o Convite do grupo) numa única transação — mesma lógica usada para criar e editar (CLAUDE.md, seção 12.1). Remover um acompanhante sempre desvincula (party_id = null), nunca exclui. Levanta PRIMARY_GUEST_NOT_FOUND ou GUEST_ALREADY_IN_ANOTHER_INVITE.';
