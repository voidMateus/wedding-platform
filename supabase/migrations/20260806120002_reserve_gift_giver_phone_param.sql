-- reserve_gift() ganha p_giver_phone (default null, compatível com os dois
-- call sites existentes que chamam por parâmetros nomeados) — grava o
-- telefone de quem presenteou junto do nome (p_contributor_name).
...
create or replace function reserve_gift(
  ...
  p_giver_phone text default null
)
...
begin
  ...
  insert into gift_reservations (wedding_id, gift_id, guest_id, group_id, contributor_name, message, giver_phone)
  values (v_gift.wedding_id, p_gift_id, p_guest_id, p_group_id, p_contributor_name, p_message, p_giver_phone)
